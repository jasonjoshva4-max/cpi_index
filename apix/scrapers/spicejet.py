...
class SpiceJetScraper(BaseScraper):
    ...
    def __init__(self, **kwargs):
        base_url = kwargs.pop("base_url", "https://www.spicejet.com")
        super().__init__(source_id="spicejet", base_url=base_url, **kwargs)

    def build_search_url(self, job: Job) -> str:
        return f"{self.base_url}/search?origin={job.route.origin}&destination={job.route.destination}&date={job.travel_date}"

    @staticmethod
    def parse_spicejet_json_data(data: dict[str, Any], job: Job) -> list[AirfareRecord]:
        records = []
        flights = data.get("flights") or data.get("data", {}).get("trips", [])
        for item in flights:
            carrier = item.get("carrier") or "SG"
            flight_no = str(item.get("flightNumber") or "")
            if flight_no and not flight_no.startswith("SG"):
                flight_no = f"SG-{flight_no}"
            ...
            rec = AirfareRecord(
                ...
                fare_class=item.get("fareClass", "Spicemax"),
        ...
        try:
            ...
            if json_captured:
                ...
                for d in json_captured:
                    recs.extend(self.parse_spicejet_json_data(d, job))
                return recs, None
            return [], None
